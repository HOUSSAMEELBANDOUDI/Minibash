...
//handle redirections and file operations
...
int	validate_files(t_list *token, int *lexer, int *fd_in, int *fd_out)
{
	...
	*fd_in = -2;
	*fd_out = -2;//handle redirections and file operations
	i = -1;
	temp = token;
...
static int	open_redirect(int lex, char *file, int *fd_in, int *fd_out)
{
	struct stat	file_info; // Declares a structure to hold file information
	char		*temp;

	temp = ft_strdup(file);
	if (stat(file, &file_info) == 0) //Checks if the file exists and retrieves its information.
	{
	...
	else if (lex == HEREDOC)
	{
		...
		*fd_out = open(file, O_RDWR | O_APPEND | O_CREAT, 0644); //Opens file for output redirection with append mode
...
static int	verify_permissions(int lex, char *file)
{
	...
	{
		if (access(file, W_OK) != 0)//Checks if the file is writable
		{
	...
	{
		if (access(file, R_OK) != 0) //Checks if the file is readable
		{
			ft_printf_fd(2, "%s: Permission denied\n", file);
			return (0);
		}
